...
class GlossaryQuery:
    ...
    def list_glossary_records(self, glossary_id: int, page: int, page_records: int, search: str | None = None):
        query = self.db.query(GlossaryRecord).filter(GlossaryRecord.glossary_id == glossary_id)
        if search:
